...
function UserDetailPanel({ userId }: { userId: string }) {
  ...
  if (state.status === 'error') {
    return <p role="alert">Impossible de charger ce profil.</p>;
  }

  const { user, memberships } = state.data;

  return (
    <section aria-label={`Profil de ${user.email}`}>
      <h3>{user.email}</h3>
      <p>{user.full_name}</p>
      ...
      {user.is_active && (
        confirming ? (
          <div aria-label="Confirmation de désactivation">
...
export function BackofficeView() {
  ...
  return (
    <section aria-label="Back-office">
      ...
      <form onSubmit={(event) => { void handleSearch(event); }} role="search">
        <label>
      ...
      {searchState.status === 'error' && <p role="alert">Impossible d&apos;effectuer la recherche.</p>}

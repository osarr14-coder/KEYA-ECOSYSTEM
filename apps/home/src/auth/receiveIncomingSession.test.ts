  ...
  () => {
    ...
    it(
      ...
      () => {
        ...
        expect(localStorage.getItem('keya_access_token')).toBeNull();
        // Un fragment sans rapport n'est pas de notre ressort — jamais retiré.
        expect(window.location.hash).toBe('#some-other-fragment=value');
      },
    );

    it('n\'écrase jamais un keya_active_organization_id déjà persisté (ticket 019, '
      + 'orthogonal à la session)', () => {
      localStorage.setItem('keya_active_organization_id', 'org-already-chosen');
      setHash('#access_token=my-access&refresh_token=my-refresh');

      receiveIncomingSession();

      expect(localStorage.getItem('keya_active_organization_id')).toBe('org-already-chosen');
    });
